                ...
                try:
                    with open(file_path, 'r') as json_file:
                        ...
                        for user in data['data']['districts']:
                            transaction_district = user['name']
...
top_user_dist_data = extract_transaction_data(root_directory)
print(top_user_dist_data)
...
try:
    ...
    if connection.is_connected():
        ...
        cursor.execute("TRUNCATE TABLE top_user_dist_data")

        # Insert new data into the MySQL database
        for index, row in top_user_dist_data.iterrows():
            sql_query = """
                INSERT INTO top_user_dist_data (State, Year, Quarter, District, Registered_users)
                VALUES (%s, %s, %s, %s, %s)
            """
